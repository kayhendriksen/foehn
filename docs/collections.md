...
| `climate_normals` | C6 | **Station normals** -- 30-year reference averages for 1961--1990 and 1991--2020. Monthly values per station. Ships as a single ZIP rather than a STAC collection, so it downloads and converts but is not available through `load()`. | TXT -> Parquet |
