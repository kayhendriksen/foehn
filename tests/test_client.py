...
def _fake(items=None, *, collection=None, body=b"a;b\n1;2\n"):
    ...
    if collection is not None:
        fake.any_collection = collection
...
def test_download_collection_detects_csv_with_query_string(tmp_path):
...
def test_download_collection_re_encodes_to_utf8(tmp_path):
    url = "https://data.geo.admin.ch/ogd-smn_tst_d_recent.csv"
    fake = _fake([_stac_item(url)])
    # Windows-1252 encoded content (ä = 0xe4)
    _serve(fake, b"col\n\xe4\n")

    download_collection("smn", tmp_path / "bronze", fetcher=fake)

    content = (tmp_path / "bronze" / "smn" / "ogd-smn_tst_d_recent.csv").read_text(encoding="utf-8")
    assert "ä" in content


def test_download_collection_saves_etag(tmp_path):
    url = "https://data.geo.admin.ch/ogd-smn_tst_d_recent.csv"
    fake = _fake([_stac_item(url)])
    _serve(fake, etag='"v1"')
...
def test_download_collection_skips_unchanged_asset(tmp_path):
...
def test_download_collection_ignores_stored_etag_when_file_is_gone(tmp_path):
...
def test_download_collection_drops_etag_when_server_stops_sending_one(tmp_path):
...
def test_download_collection_deduplicates_same_destination(tmp_path):
...
def test_download_collection_state_dir_overrides_default(tmp_path):
    ...
    state.mkdir()
    download_collection("smn", tmp_path / "bronze", state_dir=state, fetcher=fake)

    assert load_etags(state) == {url: '"abc"'}
    assert not (tmp_path / "_etags.json").exists()


def test_download_collection_prunes_stale_etags(tmp_path):
    ...
    download_collection("smn", tmp_path / "bronze", data_types=["recent"], fetcher=fake)
...
def test_download_collection_incremental_run_does_not_prune(tmp_path):
    ...
    download_collection("smn", tmp_path / "bronze", since="2026-01-01T00:00:00Z", fetcher=fake)

    assert load_etags(tmp_path)[stale] == '"keep"'


def test_download_collection_since_filter(tmp_path):
    ...
    result = download_collection("smn", tmp_path / "bronze", since="2026-01-01T00:00:00Z", fetcher=fake)
...
def test_download_collection_resilient_to_single_failure(tmp_path):
...
def test_download_grib2_saves_binary(tmp_path):
    href = "https://data.geo.admin.ch/forecast.grib2"
    fake = _fake([_stac_item(href)], body=b"GRIBdata")

    result = download_grib2("forecast_icon_ch1", tmp_path / "bronze", fetcher=fake)
...
def test_download_grib2_skips_existing_file(tmp_path):
    href = "https://data.geo.admin.ch/forecast.grib2"
    fake = _fake([_stac_item(href)], body=b"GRIBdata")
    ...
    download_grib2("forecast_icon_ch1", tmp_path / "bronze", fetcher=fake)

    assert fake.streams == []
    assert (out_dir / "forecast.grib2").read_bytes() == b"existing"


def test_download_grib2_reads_only_the_newest_page(tmp_path):
...
def test_download_netcdf_saves_nc_file(tmp_path):
    fake = _fake([{"id": "g1", "assets": {"data": {"href": "https://data.geo.admin.ch/grid.nc"}}, "properties": {}}])
    fake.default_body = b"\x89HDF"

    result = download_netcdf("surface_derived_grid", tmp_path / "bronze", fetcher=fake)
...
def test_download_netcdf_skips_existing_file(tmp_path):
    fake = _fake([{"id": "g1", "assets": {"data": {"href": "https://data.geo.admin.ch/grid.nc"}}, "properties": {}}])
...
def test_download_netcdf_since_filter(tmp_path):
    """Items older than `since` should be skipped without any HTTP call."""
    fake = _fake(
        [
            {
    ...
    result = download_netcdf("surface_derived_grid", tmp_path / "bronze", since="2026-01-01T00:00:00Z", fetcher=fake)
